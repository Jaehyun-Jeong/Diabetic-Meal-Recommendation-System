...
def time_fourier_feature(
    ...
    df[['meal_month', 'meal_day', 'meal_weekday', 'meal_hour']] = df[feature_name].apply(time_preprocess).apply(pd.Series)

    # fourier feature
    df['sin_meal_month'] = np.sin(2 * np.pi * df['meal_month']/time_upper_bounds['month'])
    df['cos_meal_month'] = np.cos(2 * np.pi * df['meal_month']/time_upper_bounds['month'])
    ...
    df['sin_meal_weekday'] = np.sin(2 * np.pi * df['meal_weekday']/time_upper_bounds['weekday'])
    df['cos_meal_weekday'] = np.cos(2 * np.pi * df['meal_weekday']/time_upper_bounds['weekday'])
    df['sin_meal_hour'] = np.sin(2 * np.pi * df['meal_hour']/time_upper_bounds['hour'])
    df['cos_meal_hour'] = np.cos(2 * np.pi * df['meal_hour']/time_upper_bounds['hour'])
...
def add_time_info_data():
    ...
    features = [
        'meal_time', 'carbs', 'calories', 'protein', 'fat', 'g0',
